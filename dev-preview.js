...
const {
    promises: { readFile, writeFile, readdir, stat }
} = require("fs")
...
app.use(async function (ctx, next) {
    const pathReg = /^\/api\/(.*)/ //  api请求
    const elementReg = /^\/element\/(.*)/ // 元素请求
    if (pathReg.test(ctx.path)) {
            ...
            await Promise.all(
                comps.map(async comp => {
                    ...
                    if (stats.isDirectory() && !compJson.includes(comp)) {
                        let jsonPpath = dirpath + "/" + comp + ".json",
                            compJson
                        try {
        ...
        let match = ctx.path.match(elementReg) // 返回匹配到的内容
        let api = match[1] //
        let result = ""
        if (api == "elementlist") {
            //获取元素列表
