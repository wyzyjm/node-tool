...
const {
    promises: { readdir, writeFile, readFile, stat }
} = require("fs")
...
async function upVersion(compId, body) {
    ...
    let versionMessagePath = `${compPath}version.json`,
        versionJson
    try {
...
async function initComp(compId) {
    let compPath = `${COMP_PATH}/${compId}/` // 组件路径
    let compJsonFilePath = `${compPath + compId}.json` // 组件json
    let compPanelFilePath = `${compPath + compId.split("-")[0]}.json` // 面板
    let mockFilePath = compPath + "mockData.json" // mock数据
    let tmplPath = compPath + "tmpl.hbs" // handlebars模板
    let emptyFilePath = [compPath + "base.css", compPath + "S.css", compPath + "L.css", compPath + "M.css"] // 样式文件
    // 判断是否已存在
    if (fs.existsSync(compPath)) {
        logger.warn(`The component "${compId}" already exists`)
        return
    }
    createDir(compPath) // 创建文件
    defaultJson.compId = compId
    await writeFile(compPanelFilePath, JSON.stringify(panelJson, null, 2), "utf8") // 写面板内容
    await writeFile(compJsonFilePath, JSON.stringify(defaultJson, null, 2), "utf8") // 组件json
    await writeFile(mockFilePath, JSON.stringify(mockData, null, 2), "utf8") // mock数据
